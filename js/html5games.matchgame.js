...
// an array to store the deck, it is bascially a clone of the matchingGame.deck.   deck--整副牌的意思
...
$(function(){	
	...
	for(var i=0;i<11;i++){
        ...
        /*clone函数来复制纸牌元素。在复制目标元素之后，调用appendTo函数将复制的纸牌元素作为子节点追加到cards元素后面*/
	}
	...
	$("#cards").children().each(function(index)
    {
		...
		$(this).css({
			"left" : ($(this).width()  + 20) * (index % 4),  /* index%4 取余数*/
			"top"  : ($(this).height() + 20) * Math.floor(index/4)    /* index/4 取整*/
		});
	...
	if (savedObject != undefined)
	{
		matchingGame.savingObject.removedCards = savedObject.removedCards;     // find those cards and remove them.
		for(var i in matchingGame.savingObject.removedCards) // i 取 removeCards 里面的卡片
		{			
			$(".card[data-card-index="+matchingGame.savingObject.removedCards[i]+"]").remove(); //++写入数据里的值
		}
	}


	matchingGame.elapsedTime = 0;     // reset the elapsed time to 0.
	
	// restore the saved elapsed time
	if (savedObject != undefined)    //用户的浏览器不支持它
	{
...
function gameover() {
    ...
    var lastScore = localStorage.getItem("last-score");                 // check if there is no any saved record
    var BestScore = localStorage.getItem("last-score");
    BestScoreObj = JSON.parse(lastScore);
    ...
    if (BestScoreObj >> lastScoreObj) {
        BestScoreObj = lastScoreObj;
    }
    ...
    $(".saved-time").html(savedTime);
        ...
        if (minutes < 10) minutes = "0" + minutes;   //给时间补位
        var seconds = currentTime.getSeconds();
        // add padding 0 to seconds
        if (seconds < 10) seconds = "0" + seconds;

        var now = day + "/" + month + "/" + year + " " + hours + ":" + minutes + ":" + seconds;   //显示时间
...
function savedSavingObject()
{
	...
	{
		savingObject = JSON.parse(savingObject);  //parse 理解
	}
	return savingObject;
}
